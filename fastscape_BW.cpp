...
///Initializing code
FastScape_BW::FastScape_BW(const int width0, const int height0)
...
void FastScape_BW::ComputeReceivers(){
...
void FastScape_BW::ComputeDonors(){
...
void FastScape_BW::FindStack(const int c, int &nstack){
...
///such an order.
void FastScape_BW::GenerateOrder(){
  int nstack=0;
  for(int c=0;c<size;c++){
    if(rec[c]==NO_FLOW){
      stack[nstack++] = c;
...
void FastScape_BW::ComputeFlowAcc(){
...
///appropriately.
void FastScape_BW::run(const int nstep){
  Tmr_Overall.start();
