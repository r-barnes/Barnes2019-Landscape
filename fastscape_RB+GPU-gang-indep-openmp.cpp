...
class FastScape_RBGPU {
 ...
 private:
  void ComputeReceivers(){
    ...
    #pragma omp target teams distribute parallel for collapse(2) default(none) //present(this,nshift[0:8],h[0:size],rec[0:size]) //default(none) present(this,h,rec,dr,nshift)
  ...
  void ComputeDonors(){
    ...
    #pragma omp target teams distribute parallel for collapse(2) default(none) // present(this,rec,nshift,donor,ndon)
    for(int y=1;y<height-1;y++)
    for(int x=1;x<width-1;x++){
      const int c = y*width+x;
      ndon[c] = 0;
  ...
  void GenerateOrder(){
    ...
    #pragma omp target enter data map(to: nstack[0:gangs])

    //TODO: Outside edge is always NO_FLOW. Maybe this can get loaded once?
    //Load cells without dependencies into the queue
    #pragma omp target teams num_teams(gangs) default(none) shared(nstack) //present(this,rec[0:size],stack[0:stack_width],nlevel[0:gangs],nstack[0:gangs])
    {
      levels[omp_get_team_num()*g_level_width] = 0;
      nlevel[omp_get_team_num()*g_level_width] = 1;

      //Introduce a local variable to prevent contention on the `nstack` array
      int mystack = omp_get_team_num()*g_stack_width;
      ...
      nstack[omp_get_team_num()] = mystack;

      int &my_nlevel      = nlevel[omp_get_team_num()*g_level_width];
    ...
    ///////////////////////////
    #pragma omp target teams num_teams(gangs) default(none) shared(nstack) //present(this,ndon[0:size],donor[0:8*size],stack[0:stack_width],levels[0:level_width],nlevel[0:gangs],nstack[0:gangs])
    {
      int level_bottom = -1;
      int level_top    = 0;
      int mystack      = nstack[omp_get_team_num()];
    ...
    #pragma omp target update from(nlevel[0:gangs])
    #pragma omp target exit data map(from:nstack[0:gangs])
  ...
  void ComputeFlowAcc(){
    #pragma omp target teams distribute parallel for default(none) //present(this,accum)
    ...
    #pragma omp target teams num_teams(gangs) default(none) //present(this,accum,levels,nshift,rec,stack,nlevel)
    {
      for(int li=nlevel[omp_get_team_num()]-3;li>=1;li--){
  ...
  void AddUplift(){
    ...
    #pragma omp target teams distribute parallel for collapse(2) default(none) //present(this,h)
    for(int y=2;y<height-2;y++)
    for(int x=2;x<width-2;x++){
      const int c = y*width+x;
      h[c]       += ueq*dt; 
  ...
  void Erode(){
    ...
    #pragma omp target teams num_teams(gangs) default(none) //present(this,levels,stack,nshift,rec,accum,h)
    {
      for(int li=0;li<nlevel[omp_get_team_num()]-1;li++){
        const int lvlstart = levels[li];
        const int lvlend   = levels[li+1];
        #pragma omp parallel for
        for(int si=lvlstart;si<lvlend;si++){
 ...
 public:
  void run(const int nstep){
    ...
    #pragma omp target enter data map(to:h[0:size],nshift[0:8],rec[0:size],ndon[0:size]) map(alloc:accum[0:size],donor[0:8*size],nlevel[0:gangs])

    //TODO: Make smaller, explain max
    stack_width = 5*size; //Number of stack entries available to each thread
    level_width = 5*size; //Number of level entries available to each thread
    ...
    #pragma omp target exit data map(from:h[0:size]) map(delete:accum[0:size],rec[0:size],ndon[0:size],donor[0:8*size],stack[0:stack_width],nlevel,levels[0:level_width])
