...
class FastScape_BWPF {
 ...
 private:
  ...
  double ComputeFlowAccHelper(const int c){
    for(int n=0;n<ndon[c];n++)
      accum[c] += ComputeFlowAccHelper(donor[8*c+n]);
    return accum[c];
  }
...
int main(int argc, char **argv){
  ...
  std::cout<<"A FastScape B&W+PQ"<<std::endl;
