...
class TerrainMorpher {
  ...
  void Erode(){
    for(unsigned int li=0;li<levels.size()-1;li++){
      for(unsigned int si=levels[li];si<levels[li+1];si++){
        ...
        newtoniters += iters;
        newtonmax    = std::max(newtonmax,iters);
      }
    }
  }
 ...
 public:
  ...
  void run(const int nstep){
    ...
    std::cout<<"Maximum Newton's method iterations = "<<newtonmax<<std::endl;
    std::cout<<"Average Newton's method iterations = "<<(newtoniters/(double)newtoncount)<<std::endl;
  }
};
